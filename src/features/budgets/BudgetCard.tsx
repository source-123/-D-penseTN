...
export function BudgetCard({ budget, onPress }: Props) {
  ...
  const remainingLabel = budget.remaining >= 0
    ? t('budgets.remaining', { amount: formatCurrency(budget.remaining) })
    : t('budgets.exceeded', { amount: formatCurrency(-budget.remaining) });
  ...
  return (
    ...
    >
      ...
      <View style={styles.bar}>
        <View style={[styles.barFill, { width: `${Math.min(budget.percent, 100)}%`, backgroundColor: barColor }]} />
      </View>
      <View style={[styles.row, isRTL && { flexDirection: 'row-reverse' }]}>
        <Text style={styles.meta}>{formatCurrency(budget.spent)} / {formatCurrency(budget.amount)}</Text>
        <Text style={[styles.meta, { color: barColor }]}>{remainingLabel}</Text>
      </View>
    </Pressable>
  );
}
